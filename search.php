		...
		while($row = mysqli_fetch_array($usersReturnedQuery)) {
			...
			if($user['username'] != $row['username']) {
				...
				if($user_obj->isFriend($row['username']))
					$button = "<input type='submit' name='" . $row['username'] . "' class='danger' value='Remove associate'>";
				else if($user_obj->didReceiveRequest($row['username']))
					$button = "<input type='submit' name='" . $row['username'] . "' class='warning' value='Respond to request'>";
				else if($user_obj->didSendRequest($row['username']))
					$button = "<input type='submit' class='default' value='Request Sent'>";
				else 
					$button = "<input type='submit' name='" . $row['username'] . "' class='success' value='Add associate'>";

				$mutual_friends = $user_obj->getMutualFriends($row['username']) . " associates in common";
